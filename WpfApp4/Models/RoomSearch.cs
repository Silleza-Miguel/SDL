...
namespace WpfApp4.Models
{
    public class RoomSearch
    {
        ...
        public static void getDatabase(string query)
        {
            ...
            if (query.Length != 0)
            {
                using (SqlCommand command = new SqlCommand($"select top 10 * from test.dbo.Room r join test.dbo.Building b on r.roomBuilding=b.buildingID where roomName like '{query}%' and roomVideo is not null", DatabaseService.connection()))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            DatabaseRoom.Add(new Room
                            {
                                ...
                                RoomQR = Convert.ToString(reader["roomQR"] is DBNull ? null : reader["roomQR"])
                            });
